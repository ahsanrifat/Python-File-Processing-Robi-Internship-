...
workbook = xlsxwriter.Workbook(r'E:\XLSX\Expenses01.xlsx')
...
worksheet2 = workbook.add_worksheet('PD')  # Data.
...
writer = pd.ExcelWriter(r'E:\XLSX\wadww.xlsx', engine='xlsxwriter')
df.to_excel(writer,sheet_name="Data",startcol=1,startrow=3)
workbook  = writer.book
worksheet = writer.sheets['Data']
cell_format = workbook.add_format()
cell_format.set_font_size(16)
cell_format.set_border(5)
worksheet.set_row(3, df.shape[0]+3, cell_format)
...
writer = pd.ExcelWriter(r'E:\XLSX\table.xlsx', engine='xlsxwriter')
workbook  = writer.book
sheet = workbook.add_worksheet('PD')
startRow=4
startCol=0
endRow=df.shape[0]+startRow
endCol=df.shape[1]+1
...
border={'bold': True, 'font_color': 'black','border':.5,'border_color':'green'}
border1={'bold': True, 'font_color': 'black','left':1}
border2={'bold': True, 'font_color': 'black','right':1}
border3={'bold': True, 'font_color': 'black','bottom':1}

endRow4=df.shape[0]+startRow
endCol=df.shape[1]
format2 = workbook.add_format({'bg_color': '#C6EFCE','font_color': '#006100'})
worksheet.conditional_format(startRow, startRow, startCol, endCol, border)
worksheet.conditional_format(startRow+1, endRow, startCol, startCol, border1)
worksheet.conditional_format(startRow+1, endRow, endCol, endCol, border2)
worksheet.conditional_format(endRow, endRow, startCol, endCol, border3)
# for x in range(startrow,endrow+1):
#     for y in range(0,endCol+1):
df.to_excel(writer,startcol=startCol,startrow=startRow,index=False)         
writer.save()
...
workbook = xlsxwriter.Workbook(r'E:\XLSX\table.xlsx')
...
for r in df(df, index=True, header=True):
    ws.append(r)
wb.save(r'E:\XLSX\table.xlsx')    
